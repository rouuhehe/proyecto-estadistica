...
#quitamos outliers y valores atípicos
outlier <- DF[DF$`Tiempo de uso`<18,]

summary(outlier$`Tiempo de uso`)
...
Una vez seleccionados los valores válidos, lo que haremos es convertir los elementos de cada fila (que están ordenados como una lista) en TRUE o FALSE si es que están o no en el vector valores_validos. En el caso que el elemento no esté en valores_validos, se convierte en TRUE y es su valor es reemplazado por "otro".
...
DF_FactorImportante <- DF_FactorImportante %>%
  mutate(
    marca = grepl("marca", `Factor importante`),
    recomendacion = grepl("recomendacion", `Factor importante`),
    portabilidad = grepl("portabilidad", `Factor importante`),
    rendimiento = grepl("rendimiento", `Factor importante`),
    oferta = grepl("oferta", `Factor importante`),
    precio = grepl("precio", `Factor importante`),
    estetica = grepl("estetica", `Factor importante`),
    otro = grepl("otro", `Factor importante`)
  )

conteo_factores <- colSums(DF_FactorImportante[, c("marca", "recomendacion", "portabilidad", 
                                                    "rendimiento", "oferta", "precio", "estetica", "otro")])
...
Para analizar la relación, en primer lugar. Observaremos la distribución de datos relacionados al factor de mayor importancia mediante la elaboración de una tabla de frecuencias.

```{r}
#Se muestran los valores para cada factor
conteo_factores <- colSums(DF_FactorImportante[, c("marca", "recomendacion", "portabilidad","rendimiento", "oferta", "precio", "estetica", "otro")])
print(conteo_factores)

#La cantidad total de personas analizadas
nrow(DF_Efectivo)
```

De esta manera, al reunir los datos obtenemos:

| Factor de mayor importancia | Frecuencia absoluta | Frecuencia relativa |
|-----------------------------|---------------------|---------------------|
| Marca                       | 429                 | 40.82%              |
| Recomendación               | 366                 | 34.82%              |
| Portabilidad                | 287                 | 27.31%              |
| Rendimiento                 | 727                 | 69.17%              |
| Oferta                      | 244                 | 23.22%              |
| Precio                      | 537                 | 51.09%              |
| Estética                    | 184                 | 17.51%              |
| Otros                       | 19                  | 1.81%               |

De esta manera, se observa que el factor de mayor relevancia en la elección de laptops es el rendimiento. Pues el 69.17% de los estudiantes lo consideraron como respuesta. De modo que, en segundo lugar va el precio con la mita de los estudiantes. En este sentido, los demás factores si bien tiene cierta importancia porque engloban entre el 15% y 30%.

Para facilitar la visualización de los datos usamos un más gráfica.

```{r}
barplot(DF_FactorImportante,)
```
